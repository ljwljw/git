	...
	<head>
		...
		<script type="text/javascript">
			$("#tijiao").click(function(){
				   ...
				   if(name==''||studycard==''||college==''||major==''||idcard==''||lasttime==''||reason==''){
				   		alert("选项不能为空");
				   		return false;
				   }else{
				   		var data = {name:name,studycard:studycard,college:college,major:major,idcard:idcard,lasttime:lasttime,reason:reason}
				   		//调用ajax    
						var json = ajax('delay',data);
						if (json) {
							location.href="{{url('/')}}";
						}
						/*$.ajax({
						    type : "get",
						    url : "http://www.wsc.com/shixun/jiekou/laravel/public/delay",
						    data:{name:name,studycard:studycard,college:college,major:major,idcard:idcard,lasttime:lasttime,reason:reason},
						    dataType : "jsonp",
